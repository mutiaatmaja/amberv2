...
<head>
    ...
    <title>Catat Absen Kebersihan | {{ config('app.name', 'Amber') }}</title>
    <script>
...
<body class="min-h-full bg-slate-100 font-body text-slate-900">
    <div class="mx-auto flex min-h-screen w-full max-w-2xl flex-col px-4 pb-8 pt-5 sm:px-6">
        <header class="rounded-3xl bg-slate-900 px-5 py-5 text-white shadow-xl">
            <p class="text-xs font-semibold uppercase tracking-[0.3em] text-emerald-300">Absensi Kebersihan</p>
        ...
        <section class="mt-4 rounded-3xl bg-white p-5 shadow-sm">
            <form method="POST"
                action="{{ route('cleaning-attendance.store', ['token' => $token, 'pointType' => $pointType]) }}"
        ...
        <section class="mt-4 rounded-3xl bg-white p-4 shadow-sm">
            <h2 class="font-heading text-base font-bold">Absensi Sesuai Jadwal Kebersihan</h2>
