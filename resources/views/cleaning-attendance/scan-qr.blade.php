<x-layouts.app :title="'Scan QR Kebersihan | ' . config('app.name', 'Amber')" heading="Scan QR Kebersihan">
    <div class="space-y-6">
        <section class="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <div class="rounded-4xl bg-emerald-700 p-6 text-white shadow-panel sm:p-8">
                ...
                <h3 class="mt-4 text-3xl font-semibold leading-tight sm:text-4xl">
                    Scan QR absensi kebersihan langsung dari aplikasi.
                </h3>
                <p class="mt-4 max-w-2xl text-sm leading-7 text-slate-100/90 sm:text-base">
                    Kamera Ada dibawah, aplikasi terbuka, tinggal scan QR absensi kebersihan yang muncul di layar. Tidak
                    perlu buka
            ...
            <div class="mt-6 grid gap-4 lg:grid-cols-[1fr_auto]">
                <div>
                    ...
                    <input id="manualQrUrl" type="text" inputmode="url" autocomplete="off" autocapitalize="off"
                        spellcheck="false"
                        placeholder="Scan langsung dari handheld scanner atau tempel URL /absen-kebersihan/..."
                        class="block w-full rounded-2xl border-slate-300 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-950">
    ...
    <script>
        (function() {
            ...
            function normalizeScanUrl(rawValue) {
                ...
                var pathMatch = parsedUrl.pathname.match(/^\/absen-kebersihan\/([^/]+)\/([^/?#]+)$/i);

                if (!pathMatch) {
                    throw new Error('Format QR tidak sesuai. Gunakan QR absensi kebersihan Amber.');
                }
