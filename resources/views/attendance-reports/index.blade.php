<x-layouts.app :title="'Cetak Rekap | ' . config('app.name', 'Amber')" heading="CETAK REKAP">
    <div class="space-y-6">
        <section
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-700 dark:text-emerald-300">
                        Rekap Satpam per Bulan
                    </p>
                    <h3 class="mt-2 text-2xl font-semibold">Cetak Statistik + Rekap PDF</h3>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">
                        Pilih satpam dan bulan untuk mencetak laporan absensi bulanan berisi statistik, rekap harian,
                        dan rincian titik patroli.
                    </p>
                </div>

                <form method="GET" action="{{ route('attendance-reports.index') }}" class="grid gap-3 md:grid-cols-3">
                    <div>
                        <label for="satpam_id" class="mb-2 block text-sm font-medium">Satpam</label>
                        <select id="satpam_id" name="satpam_id"
                            ...
                            @foreach ($satpams as $satpam)
                                <option value="{{ $satpam->id }}" @selected((int) request('satpam_id', $selectedSatpam?->id) === $satpam->id)>
                                    {{ $satpam->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <section
                ...
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        ...
                        <h4 class="mt-2 text-xl font-semibold">{{ $report['satpam']->name }} -
                    ...
                    <a href="{{ route('attendance-reports.download', ['satpam_id' => $report['satpam']->id, 'month' => $report['month']->format('Y-m')]) }}"
        ...
        @else
            <section
                ...
                Belum ada data absensi satpam untuk ditampilkan.
            </section>
        @endif
    </div>
</x-layouts.app>
