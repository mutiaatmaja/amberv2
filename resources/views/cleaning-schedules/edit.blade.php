<x-layouts.app :title="'Ubah Jadwal Kebersihan | ' . config('app.name', 'Amber')" heading="Ubah Jadwal Kebersihan">
    <div class="mx-auto max-w-3xl">
        <section
            class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <h3 class="text-xl font-semibold">Form Ubah Jadwal Kebersihan</h3>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Perbarui jadwal checkin, checkout,
                istirahat IN, dan istirahat OUT.</p>
            ...
            <form method="POST" action="{{ route('cleaning-schedules.update', $schedule) }}" class="mt-6 space-y-5"
                data-progress-form data-progress-message="Memperbarui jadwal kebersihan...">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="user_id" name="user_id" required
                        ...
                        @foreach ($cleaningUsers as $cleaningUser)
                            <option value="{{ $cleaningUser->id }}" @selected((string) old('user_id', $schedule->user_id) === (string) $cleaningUser->id)>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="break_in_time" name="break_in_time" type="time"
                            value="{{ old('break_in_time', \Illuminate\Support\Carbon::parse($schedule->break_in_time)->format('H:i')) }}"
                            required
                            class="block w-full rounded-2xl border-slate-300 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    </div>
                    <div>
                        ...
                        <input id="break_out_time" name="break_out_time" type="time"
                            value="{{ old('break_out_time', \Illuminate\Support\Carbon::parse($schedule->break_out_time)->format('H:i')) }}"
                            required
                            class="block w-full rounded-2xl border-slate-300 px-4 py-3 text-sm focus:border-emerald-500 focus:ring-emerald-500">
                    </div>
                </div>
